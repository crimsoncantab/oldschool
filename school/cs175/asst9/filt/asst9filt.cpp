...
{
    {"./shaders/texmapping0.vert", "./shaders/texmapping0.frag"},
    {"./shaders/texmapping1.vert", "./shaders/texmapping1.frag"}
};
...
static void display()
{
	...
	safe_glUniform1f(SState[active_shader].h_uTw_, texture_width);
    safe_glUniform1f(SState[active_shader].h_uTh_, texture_height);
...
static void keyboard(unsigned char key, int x, int y)
{
	switch (key)
	{
    	...
    	case 'h':
            ...
            std::printf("1\t\tUses default shader (GL_NEAREST)\n");
            std::printf("2t\tUses default shader (GL_LINEAR)\n");
            std::printf("3\t\tUses your custom shader (GL_NEAREST)\n");
            std::printf("4\t\tUses your custom shader (GL_LINEAR)\n");
...
//    glEnable(GL_CULL_FACE);
    ...
    packed_pixel_t * pixdata = ppmread("reachup.ppm", &texture_width, &texture_height);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture_width, texture_height, 0, GL_RGB, GL_UNSIGNED_BYTE, pixdata);
...
{  
    ...
    for (std::size_t i = 0; i < SState.size(); ++i)
    {
        ...
        SState[i].h_uTw_ = safe_glGetUniformLocation(SState[i].h_program_, "uTw");
        SState[i].h_uTh_ = safe_glGetUniformLocation(SState[i].h_program_, "uTh");
    }
