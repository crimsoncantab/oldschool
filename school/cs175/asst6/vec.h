...
public:
	...
	double length()													{T r(0); for (int i = 0; i < n; i++) r += d_[i] * d_[i]; return std::sqrt(r);}
	...
	Vector lerp(Vector v2, double t) {
		return (*this)*(1.-t) + v2 * t;
	}
	Vector bezerp(Vector v2, Vector prev, Vector post, double t) {
		Vector vals[4];
		vals[0] = (*this);
		vals[1] = (*this) + (v2 - prev) * (1./6.);
		vals[2] = v2 - (post - (*this)) * (1./6.);
		vals[3] = v2;
