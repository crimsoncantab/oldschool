...
struct Color {
   ...
   * @param[in] heat heat value (0 = cold, 1 = hot)
   *
   * The resulting color is a fully-saturated bright color ranging from blue
   * for cold to red for hot. */
  static Color make_heat(float v) {
    v = std::min(1.f, std::max(0.f, v));
    return make_hsv(0.02f + 0.76f * v, 1, 1);
  }
}; // end Color
