...
public:
	...
	Matrix4 getTranslation() const											{ assert(is_affine()); Matrix4 r = Matrix4(); for (int i = 0; i < 3; ++i) r[i][3] = (*this)[i][3]; return r; }
	Matrix4 getRotation() const												{ assert(is_affine()); Matrix4 r(*this); for (int i = 0; i < 3; ++i) r[i][3] = 0; return r; }
