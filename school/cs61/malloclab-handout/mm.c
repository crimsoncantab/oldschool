...
void *mm_malloc(size_t size)
{
    ...
    char *pb, *pbest; 
    unsigned int this_size, best_size, excess, index;
    ...
    if (pbest)	// if a block was found
	{
        ...
        if (excess)
		{
		    ...
		    PUT(PHDR(pbest), PACK(size, 1));
		    PUT(PFTR(pbest), PACK(size, 1));
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    size_t nextsize, excess;
    size_t oldsize = GET_SIZE(PHDR(ptr));
    size_t increase = size - oldsize;
    if (increase > 0) {
        pnext = PNEXT_BLK(ptr);
        nextsize = GET_SIZE(pnext);
        if (!GET_ALLOC(PHDR(ptr)) && nextsize >= increase) {
            remove_from_list(pnext);
            excess = nextsize-increase;
            if (excess)
		    {
                // mark *pbes as allocated
                PUT(PHDR(ptr), PACK(size, 1));
                PUT(PFTR(ptr), PACK(size, 1));
                
                ptr += size;
                
                // mark the start of pbest as a free block of size excess
                PUT(PHDR(ptr), PACK(excess, 0));
                PUT(PFTR(ptr), PACK(excess, 0));
                push(ptr);    // place the start of pbest in a list
            ...
            pnext = mm_malloc(size);
            memcpy(pnext, ptr, size - OVERHEAD);
            mm_free(ptr);
            ptr = pnext;
        }
    }
    ...
    if (newptr)
	{     
		copySize = GET_SIZE(PHDR(oldptr)) - OVERHEAD;
		if (size < copySize)
		  copySize = size;
