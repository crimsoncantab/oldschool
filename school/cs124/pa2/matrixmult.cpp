...
public:
    ...
    : data_split(m->data_split), dim(m->dim / 2), input_dim(m->input_dim), layout(m->layout), c_off(m->c_off + c_off_),
    ...
    SqMatrix(int input_dim_, int in_mem_dim_, int * data_, DataLayout data_layout_) : data_split(in_mem_dim_), data(data_),dim(in_mem_dim_), input_dim(input_dim_), c_off(0), r_off(0), layout(data_layout_) {
        should_del = false;
...
//    }
    ...
    SqMatrix(int dim_, DataLayout data_layout_) : dim(dim_), data_split(dim_), input_dim(dim_), c_off(0), r_off(0), layout(data_layout_) {
        data = new int[data_split * data_split];
        should_del = true;
    }
    ...
    void setAsSumOf(SqMatrix & a, SqMatrix & b, bool add) {
        if (layout == MORTON && a.layout == MORTON && b.layout == MORTON) {
            ...
            for (int i = 0; i < size; i++) {
                int val = a.data[i]; // + add ? b.data[i] : -(b.data[i]);
                if (add) val += b.data[i];
                else val -= b.data[i];
                data[i] = val;
            }
...
void hybrid(SqMatrix * A, SqMatrix * B, SqMatrix * C, SqMatrix * work) {
    //    if (A->isAllPadding() || B->isAllPadding()) {
    //        C->clear();
    //        return;
    //    }
    if (C->dim <= crossover) cachingConventionalMult(A, B, C);
        ...
        SqMatrix a(A, 0, 0), b(A, 0, dim2), c(A, dim2, 0), d(A, dim2, dim2),
                ...
                c11(C, 0, 0), c21(C, dim2, 0), c12(C, 0, dim2), c22(C, dim2, dim2),
                d11(work, 0, 0), d21(work, dim2, 0), d12(work, 0, dim2), d22(work, dim2, dim2);
        //        if ((f.isAllPadding() && h.isAllPadding()) || a.isAllPadding()) {
        //            c12.clear();
        //        } else {
        d11.setAsSumOf(f, h, false);
...
int calc_opt_padding(int dim) {
    int next2 = dim;
    pow2(next2);
    if (dim == next2 || crossover > dim) { //we're done
        return dim;
    } else {
...
void run_normal_mode(int argc, char** argv) {
    ...
    SqMatrix a(dim, newdim, data, my_layout);
    SqMatrix b(dim, newdim, &(data[dim2]), my_layout);
    SqMatrix c(dim, newdim, &(data[dim2 * 2]), my_layout);
    SqMatrix d(dim, newdim, &(data[dim2 * 3]), my_layout);
...
void run_experimental_mode(int argc, char** argv) {
    ...
    int new_dim;
    if (mode == HYBRID && (my_layout != SqMatrix::MORTON)) {
        new_dim = calc_opt_padding(dim);
//        cout << "new_dim:  " << new_dim << "  crossover: " << crossover << endl;
//        exit(EXIT_SUCCESS);
    } else {
        new_dim = dim;
        pow2(new_dim);
    }
    int dim2 = new_dim * new_dim;
    ...
    SqMatrix a(dim, new_dim, data, my_layout);
    SqMatrix b(dim, new_dim, &(data[dim2]), my_layout);
    SqMatrix c(dim, new_dim, &(data[dim2 * 2]), my_layout);
    ...
    switch (mode) {
        ...
        {
            SqMatrix d(dim, new_dim, &(data[dim2 * 3]), my_layout);
