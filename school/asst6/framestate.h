...
public:
	...
	void draw(ShaderState glAccess, bool pickMode) {
		...
		if (!(pickMode || isAnim_)) arcball_->draw(glAccess, eyePoseInverse);
	}
	...
	void bezerpFrames(KeyFrame & f1, KeyFrame & f2, KeyFrame & prev, KeyFrame & post, double t) {
		KeyFrame::bezerpFrames(f1, f2, prev, post, t);
		currentEye_ = objects_[eyePoseIndex_];
	}
	void startAnim() {
		isAnim_ = true;
