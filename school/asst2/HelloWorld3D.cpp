...
static int worldSkyCoordSystem = true;													//whether to use sky-sky or sky-world coord system
...
static vector <Object3D*> visibleObjects;													// the objects that are drawn
...
static Matrix4 auxilaryFrame;																// auxilary frame for current mutable

//populates 3d environment
static void initShapes() {


	Object3D* floor = new Floor(Vector3(0.1, 0.95, 0.1), floor_y, floor_size);
	Object3D* redCube = new Cube("Cube 1", Matrix4::makeTranslation(Vector3(-1.0, 0.0, 0.0)), Vector3(1, 0, 0), 1);
	Object3D* blueCube = new Cube("Cube 2", Matrix4::makeTranslation(Vector3(1.0, 0.0, 0.0)), Vector3(0, 0, 1), 1);
	Object3D* skyCam = new SkyCam(Matrix4::makeTranslation(Vector3(0.0, 0.25, 4.0)));
	...
	//cameras.push_back(skyCam);
	//cameras.push_back(redCube);
	//cameras.push_back(blueCube);
	currentEyePose = 0;
}

static void calcAuxFrame() {
	Matrix4 eyeFrame = mutableObjects[currentEyePose]->getFrame();
	Matrix4 objectFrame = mutableObjects[currentMutable]->getFrame();
	Matrix4 translation = objectFrame.getTranslationElement();
	if (currentMutable == 0) { //using skycam
		if (worldSkyCoordSystem) {
			//uses axes of skycam, origin of world
			auxilaryFrame = objectFrame.getRotationElement();
		} else {
			//uses own frame
		...
		auxilaryFrame = objectFrame.getTranslationElement() * eyeFrame.getRotationElement();
	}
}
...
static void drawStuff()
{
	...
	Matrix4 eyePose = mutableObjects[currentEyePose]->getFrame();
	Matrix4 eyePoseInverse = eyePose.getAffineInverse();
...
static void easterEgg() {
			...
			Matrix4 newFrame = Matrix4::makeTranslation(Vector3(x,y,z)) * Matrix4::makeXRotation(thetax) *
				Matrix4::makeXRotation(thetay) * Matrix4::makeXRotation(thetaz);
			Object3D* newCube = new Cube("Easter Cube", newFrame, Vector3(red, green, blue), size);
...
{
	//sign variables are used to invert behavior if manipulating the current eyePose
	double rotationSign = 1.;
	double translationSign = 1.;
	if (currentEyePose == currentMutable) {
		rotationSign = -1.;
		if (currentEyePose == 0) {
			translationSign = -1.;
		}
	...
	Object3D* object = mutableObjects[currentMutable];
	Matrix4 eyeFrame = mutableObjects[currentEyePose]->getFrame();
	Matrix4 transform;
    ...
    {
		//rotation
		transform = Matrix4::makeXRotation(-dy * rotationSign) * Matrix4::makeYRotation(dx * rotationSign);
    }
    if (mouse_click_down && mouse_rclick_button && !mouse_lclick_button)
    {
		//translation, x and y
        transform = Matrix4::makeTranslation(Vector3(translationSign * dx, translationSign * dy, 0) * 0.01);
    }
    if (mouse_click_down && (mouse_mclick_button || (mouse_lclick_button && mouse_rclick_button)))
    {
		//translation, z
        transform = Matrix4::makeTranslation(Vector3(0, 0, translationSign * -dy) * 0.01);
    }
	//apply transformation WRT aux frame
	object->preApplyTransformation(auxilaryFrame * transform * auxilaryFrame.getAffineInverse());
