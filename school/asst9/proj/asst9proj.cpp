...
static Matrix4 ProjectorFrame = Matrix4::makeTranslation(Vector3(0.0, 0.0, 2.));
...
struct ShaderState
{
    ...
    GLint h_vTexUnit0_;
    GLint h_uProjMatrixPjtr_;
    GLint h_uModelViewMatrixPjtr_;
    GLint h_uNormalMatrixPjtr_;
};
...
{
    {"./shaders/projtex.vshader", "./shaders/projtex.fshader"}
};
...
static void drawStuff()
{
    ...
    Matrix4 projector = Matrix4::makeProjection(frust_fovy, 1, frust_near, frust_far);
    projector = Matrix4::makeTranslation(Vector3(0.5,0.5,0.5)) * Matrix4::makeScale(Vector3(0.5,0.5,1)) * projector;	// textures have coordinates [0,1]x[0,1]
    projector.writeToColumnMajorMatrix(glmatrix);
    safe_glUniformMatrix4fv(SState[active_shader].h_uProjMatrixPjtr_, glmatrix);
	
    const Matrix4 PMVM = ProjectorFrame.getAffineInverse() * ObjectFrame;
    PMVM.writeToColumnMajorMatrix(glmatrix);
    safe_glUniformMatrix4fv(SState[active_shader].h_uModelViewMatrixPjtr_, glmatrix);
        
    PMVM.getNormalMatrix().writeToColumnMajorMatrix(glmatrix);
    safe_glUniformMatrix4fv(SState[active_shader].h_uNormalMatrixPjtr_, glmatrix);
...
{
    if (!mouse_lclick_button && !mouse_rclick_button) return;

    const double dx = x - mouse_click_x;
    const double dy = window_height - y - 1 - mouse_click_y;

    Matrix4& O = mouse_lclick_button ? ObjectFrame : ProjectorFrame;
    const Matrix4 Q = Matrix4::makeXRotation(-dy) * Matrix4::makeYRotation(dx);
...
static void keyboard(unsigned char key, int x, int y)
{
	switch (key)
	{
    	...
    	case 'h':
            ...
            std::printf("drag right mouse to rotate projector\n");
            break;
        case 's':
        	glFlush();
...
{  
    ...
    for (std::size_t i = 0; i < SState.size(); ++i)
    {
        ...
        SState[i].h_uProjMatrixPjtr_ = safe_glGetUniformLocation(SState[i].h_program_, "uProjMatrixPjtr");
        SState[i].h_uModelViewMatrixPjtr_ = safe_glGetUniformLocation(SState[i].h_program_, "uModelViewMatrixPjtr");
        SState[i].h_uNormalMatrixPjtr_ = safe_glGetUniformLocation(SState[i].h_program_, "uNormalMatrixPjtr");
