...
class Mesh
{
    ...
    void init_topology__()
    {
        ...
        for (std::size_t i = 0; i < face_.size(); ++i)
        {
            ...
            for (int j = 0; j < n; ++j)
            {
                ...
                {
                    Vector <int, 2>& v = E[e];
                    if (v[1] != -1) { std::cerr << "Non-manifold mesh. Exiting...\n"; exit(1); }
                    v[1] = vj;
                }
            }
        }
        ...
        for (std::map <std::pair <int, int>, Vector <int, 2> >::iterator i = E.begin(); i != E.end(); ++i, ++e)
        {
            ...
            if (i->second[1] == -1) { std::cerr << "Manifold-with-boundary mesh. Exiting...\n"; exit(1); }
            for (int j = 0; j < 2; ++j) face_[i->second[j] & ((1<<28)-1)].edge_[i->second[j] >> 28] = e | (j<<28);
        }
    }
    void resize__()
    {
    ...
    void subdivide__()
    {
        ...
        for (std::size_t i = 0; i < v_.size(); ++i) v[i].position_ = v_[i];						// v-vertices
        for (std::size_t i = 0; i < e_.size(); ++i) v[i+v_.size()].position_ = e_[i];			// e-vertices
        for (std::size_t i = 0; i < f_.size(); ++i) v[i+v_.size()+e_.size()].position_ = f_[i];	// f-vertices
        int fi = 0;
        #ifndef NDEBUG
            for (std::size_t i = 0; i < v.size(); ++i) v[i].halfedge_ = -1;
        #endif
        for (std::size_t i = 0; i < face_.size(); ++i)
        {
            findex[i] = fi;
        ...
        for (std::size_t i = 0; i < edge_.size(); ++i)
        {
            ...
            for (std::size_t j = 4*i; j < 4*i+4; ++j) for (int k = 0; k < 2; ++k) f[e[j].halfedge_[k] & ((1<<28)-1)].edge_[e[j].halfedge_[k] >> 28] = j | (k<<28);
        }
        #ifndef NDEBUG
            for (std::size_t i = 0; i < v.size(); ++i) assert(v[i].halfedge_ != -1);
        #endif
        vertex_.swap(v);
        edge_.swap(e);
        face_.swap(f);
...
public:    
    ...
    struct Edge
    {
        ...
        Vertex getVertex(const int i) const											{ assert(i >= 0 && i < 2); return Vertex(m_, m_.face_[m_.edge_[e_].halfedge_[i] & ((1<<28)-1)].vertex_[m_.edge_[e_].halfedge_[i] >> 28]); }
    ...
    Vector3 getNewFaceVertex(const Face& f) const									{ return f_[f.f_]; }
    Vector3 getNewEdgeVertex(const Edge& e) const									{ return e_[e.e_]; }
    Vector3 getNewVertexVertex(const Vertex& v) const								{ return v_[v.v_]; }

    void setNewFaceVertex(const Face& f, const Vector3& p)							{ f_[f.f_] = p; }
    void setNewEdgeVertex(const Edge& e, const Vector3& p)							{ e_[e.e_] = p; }
    void setNewVertexVertex(const Vertex& v, const Vector3& p)						{ v_[v.v_] = p; }
