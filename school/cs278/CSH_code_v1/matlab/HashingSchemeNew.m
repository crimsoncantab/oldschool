function [AnnA2B,AnnB2A,...     % results, each is 3 times the size of A or B
    ...
    = HashingSchemeNew(rgbA,rgbB,k,calcBnn,parameters,visualizationType,mask)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
dist = parameters.dist; % this is for CSH that keeps 6 pixels in x or y away from identity
...
% Output PCA projections is in class signle
if (descriptor_mode)
    ...
    WHK_with_Cb_Cr = []; % to maontaon compatibility
else
...
if (calcBnn)
    ...
    for i = 1 : k
        bestMappingNewB = (i+ max(hA,hB) + width + 1) * ones(sizA,'uint32');
...
[currFiltImgs_A, currFiltImgs_B] = ModifyBoundaries(currFiltImgs_A, currFiltImgs_B,largeVal,calcBnn,hA,hB,width,descriptor_mode , mask);
...
if (~descriptor_mode)
    ...
    % transpose at the end is much faster that strate copy.
    ...
    if (visualizeCandidates == 0 && ~visualizeDenoising)
        clear currFiltImgs_A; % Clear previouse representation of filter projections on image A
    end;
    ...
    if (visualizeCandidates == 0 && ~visualizeDenoising)
        clear currFiltImgs_B; % Clear previouse representation of filter projections on image B
    end;
    ...
    if (run_C_code)
       ...
       if (k > 1)
           ...
           alg(bestErrorsNewA, nBestMapping32uA, CF_Vectors_A, CF_Vectors_B, indices_A, indices_B, hTables_A, hTables_B, ...
            ...
            if (calcBnn)
                    alg(bestErrorsNewB, nBestMapping32uB, CF_Vectors_A, CF_Vectors_B, indices_B, indices_A, hTables_B, hTables_A, ...
                        nHashsNoe16u, nSequencyOrder16u, nSequencyLevels16u, bDownwardFlag);
            end
       end
       if (useTics)
