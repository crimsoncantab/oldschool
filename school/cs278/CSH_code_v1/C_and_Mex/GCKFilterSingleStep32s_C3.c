...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (mxGetClassID(prhs[0]) != mxINT32_CLASS || mxGetClassID(prhs[1]) != mxUINT16_CLASS ||
    ...
    mexErrMsgTxt("Input image must be a int32 matrix, kernel size must be a uint32 scalar, current alpha related kernel must be boolean, the delta step must be uint16, and the horizontal movement must be boolean!");
  ...
  pOutputImage32s = (short*)mxGetPr(plhs[0]);
...
// Original version 
//        pOutputImage32s[nCurIterStep+i] = pInputImage32s[nCurIterStep+i] + 
//           pInputImage32s[nCurIterDetlaStep+i-nColDelta] +pOutputImage32s[nCurIterDetlaStep+i-nColDelta];

//        pOutputImage32s[nSecChannelStep+i] = pInputImage32s[nSecChannelStep+i] + 
//           pInputImage32s[nSecIterDetlaStep+i-nColDelta] +pOutputImage32s[nSecIterDetlaStep+i-nColDelta];

//        pOutputImage32s[nThirdChannelStep+i] = pInputImage32s[nThirdChannelStep+i] + 
//           pInputImage32s[nThirdIterDetlaStep+i-nColDelta] +pOutputImage32s[nThirdIterDetlaStep+i-nColDelta];
     }
    }
  }
  else
